...
### 2026-06-23 — 阶段2 UI 重构

重新设计了阶段2 的 PyQt5 相机控制界面，主要改进：

- **相机实时预览**：界面左侧新增相机预览窗口，通过海康 SDK 的 `MV_CC_DisplayOneFrame` 将实时画面渲染到 Qt 原生窗口中，实验过程中可直观观察粒子状态。
- **停止实验按钮**：新增红色「■ 停止」按钮，可在实验运行中单独停止实验，而不关闭相机取流，方便连续调试。
- **浅色主题**：统一使用 Fusion 样式 + 全局浅色样式表，输入框白底深字、按钮青灰色背景、下拉框白色背景，替代原有的深色终端风格。
- **实验生命周期管理**：UI 与 Pipeline 之间通过 `set_experiment()` 注册/清除实验实例引用，支持安全停止实验，窗口关闭时自动清理实验资源。
- **布局优化**：左侧预览（stretch=3）+ 右侧控制面板（stretch=1）的水平分割布局，窗口最小尺寸扩至 900×650。
